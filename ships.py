...
class Ship:  # Parent-klasse voor alle schepen.
    ...
    def set_coordinates(self, coords):
        ...
        if len(coords) != self.length:
            raise ValueError(
                "Aantal coördinaten komt niet overeen met de lengte van het schip."
            )
    ...
    def occupies(self, row, col):
        ...
        for coord in self.coordinates:  # Loop door alle coördinaten van het schip.
            if coord == (
                row,
                col,
            ):  # Als het opgegeven coördinaat in de functie gelijk is aan de coördinaat van het schip krijg je True terug.
                return True
        return False
    ...
    def is_sunk(self, hits):
        ...
        for coord in self.coordinates:
            if (
                coord not in hits
            ):  # Als één vakje nog niet geraakt is, is het schip nog niet gezonken
                return False
        return True
