...
from string import (
    ascii_uppercase,
)  # Dit is een module die de standaard waarde van alfabet heeft, en is soms wel handig als je niet het hele ding wilt uit typen
...
class PlaatsingsUI(tk.Frame):
    ...
    def __init__(self, master, speler1_naam="Speler 1", speler2_naam="Speler 2"):
        ...
        self.grid(sticky="nsew")  # geef alle toestemmingen aan self.grid
        self.speler1_naam = (
            speler1_naam  # Zet de naam van speler 1 in een variabele -odessa
        )
        self.speler2_naam = (
            speler2_naam  # Zet de naam van speler 2 in een variabele -odessa
        )
        master.title(
            f"Zeeslag - {self.speler1_naam}: Plaats je vloot"
        )  # Zet een titel bovenaan de window, word ietsjes later over writen
        ...
        self.settings = getattr(
            master,
            "_game_settings",
            {"ships_per_player": len(SCHEEPS_SPEC), "shots_per_turn": 1},
        )

        # Basic Values instellen voor het plaatsen van schepen
        self.orientatie = tk.StringVar(
            value="H"
        )  # "H" (horizontaal) of "V" (verticaal)
        self.geselecteerde_sleutel = None  # key van gekozen schip (unieke waarde)
        self.bezet = [
            [None] * BORD_GROOTTE for _ in range(BORD_GROOTTE)
        ]  # raster met keys of None
        ...
        self.img_unknown = tk.PhotoImage(
            file=os.path.join(IMG_PAD, "Battleship_miss64.png")
        )
        self._tile_images = (
            []
        )  # zet de foto in een list om hem altijd op de juiste manier aan te kunnen roepen en als nodig te kunnen up of downscalen, kan ook zonder een list maar met meer dan 1 foto is een list altijd makkelijker
        self._tile_images.append(
            self.img_unknown
        )  # met append blijven de afbeeldingen in de list staan als we meerdere afbeeldingen willen toevoegen -odessa

        # layout van het bord zonder knoppen of functies
        paneel_links = tk.Frame(self)
        paneel_links.grid(row=0, column=0, padx=8, pady=8, sticky="ns")
        paneel_rechts = tk.Frame(self)
        paneel_rechts.grid(row=0, column=1, padx=8, pady=8, sticky="nsew")
        ...
        for sleutel, schip in self.schepen.items():
            schip["knop"] = tk.Button(
                palet,
                text=f"{schip['naam']} ({schip['lengte']})",
                bg=schip["kleur"],
                fg="white",
                activebackground={"activebackground": schip["kleur"]}.get(
                    "activebackground", schip["kleur"]
                ),  # zet een kleur voor de achtergrond van de knop
        ...
        orbox = tk.LabelFrame(paneel_links, text="Oriëntatie")
        orbox.pack(fill="x", pady=(6, 0))
        tk.Radiobutton(
            orbox, text="Horizontaal", variable=self.orientatie, value="H"
        ).grid(row=0, column=0, padx=6, pady=4, sticky="w")
        tk.Radiobutton(
            orbox, text="Verticaal", variable=self.orientatie, value="V"
        ).grid(row=0, column=1, padx=6, pady=4, sticky="w")
        tk.Label(orbox, text="Tip: druk 'r' om te roteren", fg="#666").grid(
        ...
        links.pack(
            side="left"
        )  # Om het makkelijk te houden is alles voor Links onder "links" gezet

        # Knoppen
        # Maakt een knop die alle schepen van het bord kan halen met behulp van de functie: "_reset_alle_schepen"
        tk.Button(
            links, text="🗷 Alles wissen", width=12, command=self._reset_alle_schepen
        ).pack(
            anchor="w"
        )  # width is de grote van de knop
        # Maakt een hulp knop en zet deze onder de knop van reset
        tk.Button(
            links, text="🛈       Help       ", width=12, command=self.toon_help
        ).pack(
            anchor="w", pady=(8, 0)
        )  # Deze heeft wel een width statement, want help is te kort om een standaart lengte van 9 te halen
        # Instellingen-knop
        tk.Button(
            links, text="⚙ Instellingen ", width=12, command=self._open_instellingen
        ...
        self.start_knop = tk.Button(
            actiebalk,
            width=12,
            text="Start spel",
            state="disabled",
            command=self._start_spel,
        )  # De disabeld komt omdat uit nature hij uit staat tot de waarde in "_update_start_knop" is uitgevoerd, waarna het de waarde enabled(default) krijgt
        self.start_knop.pack(
            side="right"
        )  # Deze heeft er ook de waarde right bij waardoor hij dus rechts van de linker knoppen komt
        ...
        board_wrapper.grid_rowconfigure(
            0, weight=0
        )  # Dingen met een weight van 0 zullen dus niet meetellen
        board_wrapper.grid_columnconfigure(0, weight=0)
        for r in range(1, BORD_GROOTTE + 1):
            board_wrapper.grid_rowconfigure(
                r, weight=1, minsize=CEL_GROOTTE
            )  # Dingen met een weight van 1 daartegen wel, en de minsize is de minimale groote van de cel wat dus gelijk staat aan de celgroote van de PNG
        ...
        for c in range(BORD_GROOTTE):
            tk.Label(
                board_wrapper,
                text=ascii_uppercase[
                    c
                ],  # Dit is dus simpel gezeht neem de eerste 10 stappen uit het alfabet in het lettertype TKDefaultFont en Maakt het bolt
                font=("TkDefaultFont", 10, "bold"),
            ).grid(
                row=0, column=c + 1, padx=2, pady=(0, 4), sticky="n"
            )  # Door sticky te gebruiken staan ze vast aan de knop (klikvakje) wat eronde zit, de rest is om de groote en locatie te bepalen van de cordinaten waar in de grit het beland
        ...
        for r in range(BORD_GROOTTE):
            tk.Label(
                ...
                text=str(
                    r + 1
                ),  # Hier doe je precies hetzelfde maar dan met een getal wat je elke keer +1 doet tot er geen rijen meer zijn in het speelveld
                font=("TkDefaultFont", 10, "bold"),
        ...
        self.canvas = tk.Canvas(
            board_wrapper, width=BORD_PIXELS, height=BORD_PIXELS, highlightthickness=0
        )
        self.canvas.grid(  # geef het canvas een grit om nieuwe knoppen op te zetten
            row=1,
            column=1,
            rowspan=BORD_GROOTTE,
            columnspan=BORD_GROOTTE,
        ...
        self.cell_items = (
            {}
        )  # Maakt een list om uiteindelijk makkelijk de klikknoppen in te kunnen zetten, en te kunnen wijzigen
        for r in range(BORD_GROOTTE):
            for c in range(BORD_GROOTTE):
                x0 = c * CEL_GROOTTE
                y0 = r * CEL_GROOTTE
                ...
                img_id = self.canvas.create_image(
                    x0,
                    y0,  # Cordinaten om bij te houden waar de knop/tegel zit
                ...
                self.canvas.create_rectangle(
                    ...
                    y0,
                    x0 + CEL_GROOTTE,
                    y0 + CEL_GROOTTE,  # cordinaten en lengte
                    outline="#cccccc",
                    width=1,
                    tags=(
                        "grid",
                    ),  # Kies een kleur en de breete voor het decorative lijntje
                )
        ...
        self.canvas.bind(
            "<Motion>", self._muis_beweging
        )  # Elke muisbeweging word bijgehouden om een perfecte preview te kunnen geven
        self.canvas.bind(
            "<Leave>", lambda e: self.canvas.delete("preview")
        )  # Als je het canvas verlaat (kliktegels) verlaat word de previeuw ook weg gehaald
        self.canvas.bind(
            "<Button-1>", self._linker_klik
        )  # Houd bij of je op je linker muisknop drukt om een ship te plaatsen
        ...
        self.canvas.bind(
            "<Control-Button-1>", self._rechter_klik
        )  # macOS alternatief voor rechtsklik -odessa

        master.bind(
            "r",
            lambda e: self.orientatie.set("V" if self.orientatie.get() == "H" else "H"),
        )  # Houd  bij of er op "r" word gedrukt om de oriëntatie slider te wisselen van horizontaal naar verticaal of andersom
    ...
    def _voetafdruk(self, rij, kol, lengte, orient):
        ...
        return (
            [(rij, kol + i) for i in range(lengte)]
            if orient == "H"
    ...
    def _plek_vrij(self, coords):
        ...
        return all(
            self._binnen_bord(r, c) and self.bezet[r][c] is None for r, c in coords
        )
    ...
    def toon_help(self):
        ...
        messagebox.showinfo(
            "Help",
            "Selecteer een schip links en klik op het bord om het te plaatsen.\n"
            "Klik met rechts om een schip te verwijderen.\n\n"
            "Druk op 'r' om de oriëntatie (horizontaal/verticaal) te wisselen.",
        )  # Helpfunctie tekst geschreven -odessa
        ...
        for r, c in coords:
            if self._binnen_bord(r, c):
                self._teken_cel(
                    r,
                    c,
                    outline=("#2a2" if toegestaan else "#a22"),
                    width=2,
                    fill="",
                    tags="preview",
                )
    ...
    def _linker_klik(self, e):
        ...
        if not self.geselecteerde_sleutel:
            messagebox.showinfo(
                "Kies schip", "Selecteer eerst een schip links in de balk."
            )
            return
        rij, kol = e.y // CEL_GROOTTE, e.x // CEL_GROOTTE
        schip = self.schepen[self.geselecteerde_sleutel]
        ...
        self.canvas.delete(
            "preview"
        )  # Haal de preview weg als je een ship geplaatst hebt
    ...
    def _rechter_klik(self, e):
        ...
        if (
            sleutel is None
        ):  # Er word hier gewerkt met sleutel om te kunnen kijken welk schip het is
            return
        ...
        schip["knop"].config(
            state="normal"
        )  # Reset de nu lege tegel om er een mogelijk nieuw schip op te kunnen zetten
        self._update_start_knop()
    ...
    def _herstart_met_settings(self, ships_per_player, shots_per_turn):
        ...
        if any(s["geplaatst"] for s in self.schepen.values()):
            if not messagebox.askyesno(
                "Let op", "Instellingen wijzigen reset je huidige plaatsing. Doorgaan?"
            ):
                return
    ...
    def _open_instellingen(self):
        ...
        tk.Label(frm, text=f"Aantal schepen per speler (1–{len(SCHEEPS_SPEC)}):").grid(
            row=0, column=0, sticky="w"
        )
        tk.Spinbox(
            frm, from_=1, to=len(SCHEEPS_SPEC), textvariable=ships_var, width=5
        ).grid(row=0, column=1, sticky="w", padx=(8, 0))

        # Schoten per beurt
        tk.Label(frm, text="Schoten per beurt (1–5):").grid(
            row=1, column=0, sticky="w", pady=(6, 0)
        )
        tk.Spinbox(frm, from_=1, to=5, textvariable=shots_var, width=5).grid(
            row=1, column=1, sticky="w", padx=(8, 0), pady=(6, 0)
        )

        tk.Label(frm, text="Wijzigen reset het plaats-scherm.", fg="#a00").grid(
        ...
        tk.Button(btns, text="Annuleren", command=win.destroy).pack(
            side="left", padx=(0, 6)
        )
    ...
    def _start_spel(self):
        ...
        try:
            self.master.bind(
                "<Return>", self._start_spel
            )  # dit is de oplossing voor het dicht drukken van het scherm zonder dat je door kunt -- Mattijn
        except Exception:
        ...
        p1 = Player(
            self.speler1_naam, self.vloot_speler1
        )  # Maak de spelers aan met hun naam (niet meer hardcoded als "speler 1" of "speler 2") -odessa
        p2 = Player(self.speler2_naam, vloot_speler2)
        spt = getattr(self.master, "_game_settings", {}).get(
            "shots_per_turn", 1
        )  # shots_per_turn doorgeven aan het spelbord
